...
var gv = parseInt(countUser[0].SoLuong);
var hs = parseInt(countUser[1].SoLuong);
var ph = parseInt(countUser[2].SoLuong);
...
var countHSlkOptions = {
    ...
    series: [parseInt(countHSlk[0].SoHS), parseInt(countHSlk[1].SoHS) - parseInt(countHSlk[0].SoHS)],
...
var countPHlkOptions = {
    ...
    series: [parseInt(countPHlk[0].SoPH), parseInt(countPHlk[1].SoPH) - parseInt(countPHlk[0].SoPH)],
...
for (var i = 6; i <= 16; i++) {
    ...
    var item = countAge.find(function (element) {
        if (element.Tuoi >= 16) {
            ...
            return parseInt(element.Tuoi) == i;
...
var countHSDHOptions = {
    ...
    series: [parseInt(ds_HSDD[0].so), parseInt(ds_HSDD[1].so)],
...
html += '<ul> <li> Tổng số lớp đã mở : ' + (parseInt(ds_tongLop[0].so) + parseInt(ds_tongLop[1].so)) + '</li>  ';
