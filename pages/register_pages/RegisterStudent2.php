...
if (isset($_POST['submit'])) {
    ...
    if (empty($_POST['userName'])) {
        ...
        $userName_error = "Tên đăng nhập không được có khoảng trắng";
    } else if (strlen($_POST['userName']) < 6) {
        $userName_error = "Tên đăng nhập cần ít nhất 6 ký tự";
    }
    ...
    if (empty($_POST['passWord'])) {
        ...
        $passWord_error = "Mật khẩu không được có khoảng trắng";
    } else if (strlen($_POST['passWord']) < 8) {
        $passWord_error = "Mật khẩu cần ít nhất 8 ký tự";
    }
    ...
    } else if (strpos($_POST['confirmpassWord'], ' ') !== false) {
        $confirmPassword_error = "Mật khẩu không được có khoảng trắng";
    } else if (strlen($_POST['confirmpassWord']) < 8) {
        $confirmPassword_error = "Mật khẩu cần ít nhất 8 ký tự";
    }
    ...
    if (empty($userName_error) && empty($passWord_error) && empty($confirmPassword_error)) {
        ...
        if (checkExitUser($userName, $connection)) {
            $userName_error = "Tên đăng nhập đã được sử dụng";
            $check = true;
        }
        ...
        <div class="login-student" style=" width: 500px;">
            ...
            <div class="login-student-form">
                <form action="<?php htmlspecialchars($_SERVER['PHP_SELF']) ?>" method="post">
                    ...
                    <p style="color:red ; margin-bottom: 20px;font-size : 18px" id="username-error">
                        <?php
                        if (isset($_POST['submit'])) {
                            echo "$userName_error";
                        }
                        ...
                        ?></p>
                    <div>
                        <input name="passWord" class="login-student-form-input" type="password" placeholder="Nhập mật khẩu : " value="<?php echo $passWord ?>">
                    </div>
                    <p style="color:red ; margin-bottom: 20px;font-size : 18px" id="password-error">
                        <?php
                        if (isset($_POST['submit'])) {
                            echo "$passWord_error";
                        }
                    ...
                    <div>
                        <input name="confirmpassWord" class="login-student-form-input" type="password" placeholder="Xác nhận mật khẩu : " value="<?php echo $confirmPassword ?>">
                    </div>

                    <?php
                    if ($success) {
                    ?>
                        <p style="color:lime; margin-bottom: 20px;font-size : 18px" id="repassword-error">
                            <?php
                            if (isset($_POST['submit'])) {
                                echo "$success";
                            }
                    ...
                    <?php   } else { ?>
                        <p style="color:red ; margin-bottom: 20px;font-size : 18px" id="repassword-error">
                            <?php
                            if (isset($_POST['submit'])) {
