...
function inserttk_gv($connection, $magv, $username, $pass, $ngaydk)
{
...
function updatePassGV($connection, $username, $pass, $magv)
{

    $sql = "update tk_gv set UserName = ? ,  Password = ? where MaGV =?";
    try {
        $connection->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $statement = $connection->prepare($sql);
...
function checkAccTeacher($connection, $magv)
{
    ...
    $sql = "select * from luonggv where MaGV = ?";
    try {
        $connection->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $statement = $connection->prepare($sql);
        $statement->execute([$magv]);
