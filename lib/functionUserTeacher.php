...
//     // update diem danh
function updateDateDiemDanh($malop,  $timeOld, $timeNew, $connection)
{
    $sql = "UPDATE diemdanh SET ThoiGian = ? WHERE MaLop = ? AND ThoiGian = ?";
    try {
        $statement = $connection->prepare($sql);
        $statement->bindParam(1, $timeNew);
        $statement->bindParam(2, $malop);
        $statement->bindParam(3, $timeOld);
...
{
    $sql = 'SELECT * FROM luonggv WHERE MaGV = ?  and  TrangThai = "Đã thanh toán" order by ThoiGianTT desc';
    try {
        $connection->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $statement =  $connection->prepare($sql);
        $statement->bindParam(1, $magv);
